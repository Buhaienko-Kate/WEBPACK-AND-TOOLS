...
export function onDeleteTask(event) {
  ...
  const nearestInputId = event.target.parentNode.firstElementChild.dataset.id; //   console.log(nearestInputId);

  deleteTask(nearestInputId).then(() => getTasksList()).then(newTasksList => {
